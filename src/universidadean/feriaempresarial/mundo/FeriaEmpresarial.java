...
public class FeriaEmpresarial {
    ...
    public String metodo1() {
        ...
        int zonanorte = 0;
        Boolean sw = false;
        for (Puesto ubicacion : puestos) {
            if (ubicacion.estaOcupado()) {
                for (Empresa empresa : empresas) {
                    if (ubicacion.darNombreEmpresa().equals(empresa.darNombre())) {
                        //datos += puesto.darZona() + ", empresa " + puesto.darNombreEmpresa() + " con "
                        //        + empresa.darNumeroPersonasAsistentes() + " expositores\n";
                        if (ubicacion.darNombreEmpresa().equals(empresa.darNombre())) {
                            if (ubicacion.darZona() == Puesto.ZONA_NORTE) {
                                zonanorte += empresa.darNumeroPersonasAsistentes();
                                sw = true;
                            }
                            if (ubicacion.darZona() == Puesto.ZONA_SUR) {
                                zonasur += empresa.darNumeroPersonasAsistentes();
                                sw = true;
                            }
                            if (ubicacion.darZona() == Puesto.ZONA_ORIENTE) {
                                zonaori += empresa.darNumeroPersonasAsistentes();
                                sw = true;
                            }
                            if (ubicacion.darZona() == Puesto.ZONA_OCCIDENTE) {
                                zonaocci += empresa.darNumeroPersonasAsistentes();
                                sw = true;
                            }
                            if (ubicacion.darZona() == Puesto.ZONA_CENTRO) {
    ...
    public String metodo2() {
        ...
        porcTotal = numeroActualExpositores * totalPuestos;
        porcTotal = porcTotal / 100;
        //System.out.println(numeroActualExpositores +" "+ totalPuestos + " " + porcentajeTotal);
        String resp = String.valueOf(porcTotal);
        if (porcTotal > 60){
            return resp + "% El punto de equilibrio se ha cumplido por encima del 60%";
        }else{
            return resp + "% El punto de equilibrio no se ha cumplido, esta por debajo del 60%";
        }
    }
